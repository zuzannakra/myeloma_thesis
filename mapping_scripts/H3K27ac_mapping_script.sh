...
# This script performs ChIP-seq quality control,
...
# Debugging system to test the script and its commands
DBG=0
CMD=''
if [ $DBG = 1 ] ; then
  CMD=echo
...
for filename1 in `ls ./trim/*"${filename1_suffix}"` ; do
  ...
  $CMD bowtie2 -p 4 -x /rds/general/user/zwk22/projects/kklf/live/genomes/bowtie2/hg38 \
    -1 "$filename1" \
    -2 "$filename2" \
    -S "$output_sam" --local --very-sensitive-local --no-unal --no-mixed --no-discordant --phred33   -I 10 -X 1000
...
for filename in `ls ./rmdup/*blacklist.sorted.bam` ; do 
  ...
  $CMD findPeaks $tag_name -style histone -o $outname -i $input
